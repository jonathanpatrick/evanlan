...
const SORT_KEYS = {
  ...
  cs: (c: ChampionSummary) => c.cs / Math.max(c.games, 1),
};

export function Champions() {
  const modes = useSelectedModes();
  const { data, error, loading } = useAsync(() => api.champions(modes), [modes]);
          ...
          <table>
            <thead>
              <tr>
                ...
                <SortableHeader id="cs" label="CS / game" sortId={t.sortId} sortDir={t.sortDir} onClick={t.toggleSort} className="numeric" title="Average CS per game. Live-client API doesn't expose damage totals." />
              </tr>
            </thead>
            <tbody>
              {t.rows.map((c) => (
                <tr key={c.champion_name}>
                  ...
                  <td className="numeric">
                    {fmt.int(c.cs / Math.max(c.games, 1))}
                  </td>
                </tr>
              ))}
            </tbody>
